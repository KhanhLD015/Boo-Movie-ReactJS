...
function Contents(props) {
  ...
  const handleScrollRight = () => {
    ...
    if (silderRef.current.scrollLeft < maxScrollLeft) {
      SmoothHorizontalScrolling(silderRef.current, 
        250, 
        movieRef.current.clientWidth,
        silderRef.current.scrollLeft
        );
    } else if (silderRef.current.scrollLeft === maxScrollLeft
  ...
  return (
    <MoviesRowContainer>
      ...
      <MoviesSlider ref={silderRef}>
        {movies.map((movie, index) => (
          <div key={index} className="movie-item" ref={movieRef}>
            <img src={movie} alt="" />
